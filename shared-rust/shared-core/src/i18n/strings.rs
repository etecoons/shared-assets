...
pub fn lookup(key: StringKey, lang: Language) -> &'static str {
    let entries: &[(&str, &str)] = match key {
        ...
        StringKey::StatusValidationError => &[
            ...
            (
                "es",
                "Error de validación: por favor, verifique su entrada.",
            ),
            (
                "de",
                "Validierung fehlerhaft: Bitte überprüfen Sie Ihre Eingabe.",
            ),
            ("ja", "検証に失敗しました：入力を確認してください。"),
            (
                "fr",
                "Échec de la validation : veuillez vérifier votre saisie.",
            ),
            (
                "pt",
                "Falha na validação: por favor, verifique a sua entrada.",
            ),
            (
                "ru",
